...
export default function App() {
  ...
  return (
    <div className="min-h-screen">
      ...
      <header className="border-b border-[var(--border)] relative">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-[var(--accent)] to-[var(--blue)] flex items-center justify-center font-bold text-[var(--bg-primary)] text-sm">
              P
            </div>
            <div>
          ...
          <a
            ...
            className="text-[var(--text-muted)] hover:text-[var(--text-secondary)] transition-colors text-sm flex items-center gap-1.5"
          >
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-6 grid lg:grid-cols-[380px_1fr] gap-6">
        {/* Input Panel */}
        <div className="space-y-4">
          ...
          <section className="rounded-xl border border-[var(--border)] bg-[var(--bg-card)] p-4">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-[var(--bg-secondary)] border border-[var(--border)] rounded-lg py-2.5 pl-7 pr-3 text-[var(--text-primary)] font-mono text-lg focus:outline-none focus:border-[var(--accent-mid)] transition-colors"
                placeholder="100"
                min="0"
                step="any"
              />
          ...
          <section className="rounded-xl border border-[var(--border)] bg-[var(--bg-card)] p-4">
                ...
                {outcomes.map((o) => (
                  ...
                  >
                    <input
                      ...
                      className="flex-1 min-w-0 bg-[var(--bg-secondary)] border border-[var(--border)] rounded-lg py-2 px-3 text-sm text-[var(--text-primary)] focus:outline-none focus:border-[var(--accent-mid)] transition-colors"
                      placeholder="Label"
                    />
                    <div className="relative w-20 shrink-0">
                      <input
                        ...
                        className="w-full bg-[var(--bg-secondary)] border border-[var(--border)] rounded-lg py-2 pl-3 pr-6 text-sm font-mono text-[var(--text-primary)] focus:outline-none focus:border-[var(--accent-mid)] transition-colors"
                        placeholder="50"
                        min="0.1"
                        max="99"
                        step="any"
                      />
                      <span className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[var(--text-muted)] text-xs">c</span>
                    </div>
                    {useCustomDist && (
                      <div className="relative w-20 shrink-0">
                        ...
                        <input
                          ...
                          className="w-full bg-[var(--bg-secondary)] border border-[var(--border)] rounded-lg py-2 pl-5 pr-2 text-sm font-mono text-[var(--text-primary)] focus:outline-none focus:border-[var(--accent-mid)] transition-colors"
                          placeholder="0"
                          min="0"
                          step="any"
                        />
                    ...
                    <button
                      ...
                      className="w-7 h-7 shrink-0 rounded-md flex items-center justify-center text-[var(--text-muted)] hover:text-[var(--negative)] hover:bg-[var(--negative-dim)] disabled:opacity-20 disabled:cursor-not-allowed transition-colors text-xs"
                    >
                      &times;
                    </button>
                  </motion.div>
            ...
            <button
              onClick={addOutcome}
              className="mt-3 w-full py-2 rounded-lg border border-dashed border-[var(--border)] text-[var(--text-muted)] hover:border-[var(--accent-mid)] hover:text-[var(--accent)] transition-colors text-sm"
            >
              + Add Outcome
            </button>
          </section>
          ...
          <section className="rounded-xl border border-[var(--border)] bg-[var(--bg-card)] p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-[var(--text-muted)] mt-0.5">
                  {useCustomDist ? 'Custom amounts per outcome' : 'Equal split across all outcomes'}
                </p>
              </div>
              <button
              ...
              >
                <span
                  className={`absolute top-0.5 w-4 h-4 rounded-full bg-white transition-transform ${
                    useCustomDist ? 'left-5.5' : 'left-0.5'
                  }`}
                />
              </button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                ...
                <button
                  ...
                  className="py-2 px-3 rounded-lg border border-[var(--border)] bg-[var(--bg-secondary)] text-xs text-[var(--text-secondary)] hover:border-[var(--accent-mid)] hover:text-[var(--accent)] transition-colors text-left"
                >
                  {preset.name}
                </button>
              ))}
          ...
          <AnimatePresence mode="wait">
            {result ? (
              ...
              <motion.div
                ...
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="rounded-xl border border-dashed border-[var(--border)] bg-[var(--bg-card)] p-12 text-center"
              >
      ...
      <footer className="border-t border-[var(--border)] mt-8">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between text-xs text-[var(--text-muted)]">
...
function StatCard({
  ...
  return (
    <div
      className="rounded-xl border border-[var(--border)] bg-[var(--bg-card)] p-3.5 hover:border-opacity-60 transition-colors"
    ...
    >
      ...
      <p className="font-mono text-xl font-bold" style={{ color: c.text }}>
        {value}
      </p>
...
function RoiBar({ outcome, maxRoi }: { outcome: OutcomeResult; maxRoi: number }) {
  ...
  return (
    <div className="flex items-center gap-3">
      <span className="w-20 text-xs text-[var(--text-secondary)] truncate shrink-0">
        {outcome.label}
      </span>
      ...
      <span className="w-14 text-right font-mono text-xs font-semibold" style={{ color }}>
        {fmt(outcome.roi)}x
      </span>
    </div>
  )
...
function ScenarioCard({
  ...
  return (
    <div
      className="rounded-xl border bg-[var(--bg-card)] p-4"
    ...
    >
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-sm font-medium text-[var(--text-secondary)]">{outcome.label} wins</span>
      </div>
      <div className="grid grid-cols-3 gap-3 text-center">
        <div>
          <p className="text-[10px] text-[var(--text-muted)] uppercase mb-0.5">Payout</p>
          <p className="font-mono text-sm font-bold text-[var(--blue)]">${fmtCompact(outcome.payout)}</p>
        </div>
        <div>
          <p className="text-[10px] text-[var(--text-muted)] uppercase mb-0.5">Profit</p>
          <p
        ...
        <div>
          <p className="text-[10px] text-[var(--text-muted)] uppercase mb-0.5">ROI</p>
          <p className="font-mono text-sm font-bold" style={{ color: borderColor }}>
            {fmt(outcome.roi)}x
          </p>
        </div>
      </div>
      <div className="mt-3 h-1 rounded-full bg-[var(--bg-secondary)] overflow-hidden">
        <motion.div
